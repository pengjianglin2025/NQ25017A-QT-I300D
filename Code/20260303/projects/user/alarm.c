...
static void Buzzer_TIM_Init(void)
{
    ...
    uint16_t PrescalerValue = (uint16_t)(SystemCoreClock / 16000000) - 1; 
    uint16_t Period = (SystemCoreClock / (PrescalerValue + 1) / BUZZER_PWM_FREQ) - 1;
    ...
    TIM_OCInitStructure.Pulse = Period / 2;  // 50%占空比
    TIM_OCInitStructure.OcPolarity = TIM_OC_POLARITY_HIGH;
    // 注意：OCInitType结构体中没有OcPreLoadEnable成员
    // 预加载需要使用TIM_ConfigOc2Preload函数配置
    
    TIM_InitOc2(BUZZER_TIMER, &TIM_OCInitStructure);  // 使用通道2
    TIM_ConfigOc2Preload(BUZZER_TIMER, TIM_OC_PRE_LOAD_ENABLE);  // 使能预加载
...
{
    TIM_SetCmp2(BUZZER_TIMER, 0);  // CCR=0，PWM输出恒为低电平
    buzzer_state = 0;
}
